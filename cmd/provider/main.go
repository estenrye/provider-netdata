...
import (
	...
	"github.com/estenrye/provider-netdata/apis"
	"github.com/estenrye/provider-netdata/apis/v1alpha1"
	"github.com/estenrye/provider-netdata/config"
	"github.com/estenrye/provider-netdata/internal/clients"
	"github.com/estenrye/provider-netdata/internal/controller"
	"github.com/estenrye/provider-netdata/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for NetData").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-netdata"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-netdata",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add NetData APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup NetData controllers")
